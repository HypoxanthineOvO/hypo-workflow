...
def scenario_specific(scene: Path, result: ScenarioResult) -> None:
    ...
    if name == "s01-fresh-start":
        ...
        add(result, "plugin_json", plugin.returncode == 0 and '"version": "5.1.0"' in plugin.stdout)
    ...
    elif name in {"s16-plan-discover", "s17-plan-review", "s18-template-library"}:
...
def main() -> int:
    ...
    (RESULTS_ROOT / f"{stamp}-s01-s18.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
