...
- `/hw:plan`, `/hw:plan:discover`, `/hw:plan:decompose`, `/hw:plan:generate`, `/hw:plan:confirm`, `/hw:plan:review`
...
16. After a milestone-level result is final, append one lifecycle entry to `.pipeline/log.yaml`.
17. After Plan Review, add the prompt to history and advance state to the next prompt immediately.
18. If `auto_continue=false`, stop after the state advance and wait for the user to say `继续`.
19. If there is no next prompt, mark the pipeline `completed`, persist state, and stop.
