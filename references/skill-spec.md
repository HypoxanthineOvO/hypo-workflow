...
The repository currently has 32 local Skill files under `skills/*/SKILL.md`:
...
- Root `SKILL.md` is the aggregate Hypo-Workflow router and system reference. It is not counted as a child Skill in the 32 local Skill files.
- The OpenCode command map exposes 32 user-facing commands and 31 user-facing Skill paths because `/hw:patch` and `/hw:patch fix` intentionally share `skills/patch/SKILL.md`.
...
- `skills/watchdog/SKILL.md` is a valid internal exception: it is cron-only, should not be user-invocable, and should not be counted among the 31 user-facing Skill paths.
