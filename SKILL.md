...
9. Mark the selected step as `running`, record `started_at`, set `current.phase=executing`, record the resolved executor, and append a step-start log event.
...
12. Record notes, timing, actual executor, result, and `PROGRESS.md` updates.
...
20. If there is no next prompt, set `current.phase=completed`, mark the pipeline `completed`, persist state, and stop.
