...
\begin{example}{案例插框：Hypo-LaTeX 的 Prompt 搬运阶段}{case:hypo-latex}
...
\begin{example}{案例插框：Bill 的界面修补泥潭}{case:bill}
\textbf{项目名：}Bill。\\
...
\begin{example}{案例插框：Agent 与 Superpowers 的阶段性成功}{case:agent}
\textbf{项目名：}Agent。\\
...
\begin{example}{案例插框：Research / Info 的搬运工阶段}{case:research-info}
...
\caption{AI Coding 约束到系统需求的映射}\label{tab:constraints}\\
\toprule
...
\begin{figure}[H]
  ...
  \caption{GPT Image 2 / Image Gen 润色后的 file-first architecture 视觉版本}
\end{figure}
...
\begin{figure}[H]
  \centering
  \input{figures/opencode-adapter.tex}
...
\begin{figure}[H]
  \centering
  \input{figures/progressive-discover.tex}
...
\begin{figure}[H]
  \centering
  \input{figures/test-profile-matrix.tex}
...
\begin{longtable}{p{0.18\linewidth}p{0.24\linewidth}p{0.24\linewidth}p{0.24\linewidth}}
\caption{C2 Validation Matrix}\label{tab:validation}\\
\toprule
能力 & 关键证据 & 自动化验证 & 备注 \\
\midrule
\endfirsthead
...
\midrule
\endhead
README 自动更新 & \texttt{templates/readme-spec.md}、README managed blocks & \texttt{core/test/readme-update.test.js}；\texttt{checkReadmeFreshness()} & 发版路径要求通过 freshness gate \\
Skill 质量治理 & \texttt{references/skill-spec.md}、Skill frontmatter/headings & \texttt{core/test/skill-quality.test.js}；\texttt{checkSkillQuality()} & 当前仓库结果为 0 issues / 31 Skill 文件 \\
OpenCode 状态面板 & status runtime、sidebar/footer plugin & \texttt{core/test/opencode-status.test.js}；\texttt{core/test/opencode-panels.test.js} & 只读投影，不直接改 workflow 状态 \\
Batch Planning & Feature Queue、Markdown artifact、Mermaid/TikZ 图 & \texttt{core/test/batch-plan.test.js}；\texttt{core/test/feature-queue-ops.test.js} & 普通单功能 \texttt{/hw:plan} 行为保持不变 \\
Chat Mode & \texttt{chat:} state、\texttt{chat\_entry}、Stop/SessionStart Hook & \texttt{core/test/chat-*.test.js} & 轻量讨论与主 Milestone 流分离 \\
Progressive Discover & category/effect/verification big questions & \texttt{core/test/progressive-discover.test.js} & 支持 optional \texttt{@karpathy/guidelines} \\
Test Profiles & webapp / agent-service / research evidence contracts & \texttt{core/test/test-profile.test.js} & 验证策略前置到 Plan 阶段 \\
...
\begin{longtable}{p{0.15\linewidth}p{0.1\linewidth}p{0.1\linewidth}p{0.12\linewidth}p{0.1\linewidth}p{0.13\linewidth}p{0.13\linewidth}p{0.12\linewidth}}
\caption{AI 工具能力对比}\label{tab:compare}\\
\toprule
工具 & Repo visibility & Planning ability & Context persistence & Recoverability & Style solidification & Automatic maintenance & Validation loop \\
\midrule
\endfirsthead
\toprule
工具 & Repo visibility & Planning ability & Context persistence & Recoverability & Style solidification & Automatic maintenance & Validation loop \\
...
Notion AI Opus + Codex & 强 & 强 & 强 & 中上 & 中 & 弱 & 中上 \\
