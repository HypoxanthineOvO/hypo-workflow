...
\begin{frame}{File-First Architecture}
  \centering
  \includegraphics[width=0.86\linewidth,height=0.72\textheight,keepaspectratio]{assets/gpt-image/file-first-architecture.png}
\end{frame}
...
\begin{frame}{为什么用命令来组织，而不是只列 Feature}
  ...
  \end{block}
  \begin{alertblock}{视觉边界}
    GPT Image 2 / Image Gen 只负责封面、分节页和概念架构的视觉润色；真实主张仍然由命令、YAML、测试、日志、PDF 和状态面板作证。
  \end{alertblock}
  \begin{columns}[T,totalwidth=\textwidth]
    \column{0.31\textwidth}
    \begin{block}{入口}
